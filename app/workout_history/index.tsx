...
import {
    ...
    getWorkoutPreviewsByDate,
} from "@/utils/database/databaseGetters";
import { unixToDate } from "@/utils/dateUtils";
import {
    CompleteExerciseDate,
    WorkoutFromDB,
...
const WorkoutHistory = () => {
    ...
    const markedDates = useCalendarStore((state) => state.markedDates); // also is cache for all needed calendar info
    const setMarkedDates = useCalendarStore((state) => state.setMarkedDates);
    ...
    const processPreviewData = (previewData: WorkoutFromDB[]) => {
        ...
        const newMarked = previewData.reduce(
            (acc: Record<string, { dots: any[] }>, workout) => {
                ...
                if (dateStr in acc) {
                    acc[dateStr].dots.push({
                        key: workout.date,
                        color: workout.tag_color,
                    });
                } else {
                    acc[dateStr] = {
                        dots: [{ key: workout.date, color: workout.tag_color }],
                    };
                }
                return acc;
            },
    ...
    const handleDayPress = (calendarDate: CalendarDate) => {
        ...
        const newMarkedDates: Record<string, object> = {
            ...markedDates,
            [calendarDate.dateString]: {
                ...(markedDates[calendarDate.dateString] || {}),
                selected: true,
        ...
        // Update selected date, triggering re-render
        setSelectedDate(calendarDate.dateString);
    };

    // Workout preview updates on refresh trigger or date change
    useEffect(() => {
        const previews = getWorkoutPreviewsByDate(selectedDate || "");
    ...
    const handleSearch = () => {
        ...
        const data =
            soughtVariant === ""
                ? getRecentExercises(soughtExercise)
                : getRecentExercises(soughtExercise, soughtVariant);
        setExercises(data);
    };
                            ...
                            <>
                                ...
                                <View className="h-96 flex-row flex-wrap gap-x-4">
                                    {workoutPreviews.map((preview) => (
                                        <WorkoutPreview
                                            key={preview.date}
                                            workoutId={preview.date}
                                            readable_date={unixToDate(
                                                preview.date
                                            ).toDateString()}
                                            color={preview.tag_color}
                                        />
                                    ))}
                                </View>
                            </>
                            ...
                            <>
                                <View className="min-h-60 items-center -mb-0">
                                    <AutocompleteSelect
                                        ...
                                        setter={setSoughtExercise}
                                        placeholder={"Search by exercise"}
                                    />
                                    <AutocompleteSelect
                                        size="small"
                                        optionType={"variants"}
                                        setter={setSoughtVariant}
                                        placeholder={"Search by variant"}
                                    />
                                    <Pressable
                                        className="p-4 m-4 rounded-full bg-[#FFDD80] w-40 items-center"
