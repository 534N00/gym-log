...
const SetControls: React.FC<SetControlsProps> = ({ exerciseId, store, editable=false }) => {
    const thisExercise = store((state: StoreState) => state.exercises[exerciseId]);
    ...
    return (
        <View className="flex-col items-center -mb-12">
            ...
            >
                ...
                <Checkbox label={"Unilat"} checked={!!thisSet?.is_uni} editable={editable} onCheck={(check) => updateSet(exerciseId, thisSet.id, { is_uni: check === true ? 1 : 0 })}/>
            </View>
        </View>
    );
};
